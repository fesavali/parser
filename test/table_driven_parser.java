...
public class table_driven_parser {
    ...
    private static int term() {
        System.out.println("area and parameter output:");
        s.indentPrint(1, "term", print);
        int subtotal = factor();
        while (token == (int) '*' || token == (int) '/' || token == (int) '^' || token == (int) '%') {
            int saveOp = token;
            token = lex.getToken();
            if ((int) saveOp == '*') subtotal *= term();
            else if ((int) saveOp == '/') subtotal = subtotal / term();
            else if ((int) saveOp == '^') subtotal = (int) Math.pow(subtotal,term());
            else if ((int) saveOp == '%') subtotal = subtotal % term();
            else {
                System.out.println("error: " + (char) saveOp + "  expected '*'  or '/' ");
            }
